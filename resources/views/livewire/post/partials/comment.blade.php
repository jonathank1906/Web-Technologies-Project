<div wire:key="comment-{{$comment->id}}" class="flex items-center gap-3 py-2">
    ...
    <img src="{{ $comment->user->getProfilePictureUrl() }}" alt="{{ $comment->user->name }}'s profile"
    ...
    <div class="grid grid-cols-7 w-full gap-2">
        ...
        <div class="col-span-1 flex text-right justify-end mb-auto">
            <button class="font-bold text-sm ml-auto"
                @if(auth()->id() === $comment->user_id) disabled @else wire:click='toggleCommentLike({{$comment->id}})' @endif>
                {{-- svg --}}
